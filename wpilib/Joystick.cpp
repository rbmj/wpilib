	...
	, m_buttons (NULL)
{
	...
	ButtonValues = {0}; //Initialize all button values to false
}
...
bool Joystick::ButtonPressed(UINT32 button) {
	if (ButtonValues[button] == false and this.GetRawButton(button) == true) {
	/*Returns true if and only if the button was not pressed the previous call and is pressed the current call*/
